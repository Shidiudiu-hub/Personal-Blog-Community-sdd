  ...
  <div class="my-collects">
    <el-card>
      ...
      <div v-loading="loading" class="article-list">
        <el-empty v-if="!loading && list.length === 0" description="暂无收藏的文章" />
...
async function loadList() {
  loading.value = true
  try {
    const res = await ArticleApi.myCollects({ page: page.value, size: size.value })
    console.log('收藏列表原始响应:', res)
    ...
    console.log('收藏列表数据:', list.value, '总数:', total.value)
  } catch (e) {
